...
resource "google_compute_instance_template" "default" {
  ...
  disk {
    source_image = "${var.disk1_source_img}"
    auto_delete  = "${var.disk1_disk_auto_delete}"
    disk_type    = "${var.disk1_disk_type}"
  ...
  disk {
    source_image = "${var.disk1_source_img}"
    auto_delete  = "${var.disk2_disk_auto_delete}"
    disk_type    = "${var.disk2_disk_type}"
  ...
  metadata = "${merge(
    map("startup-script", "${var.startup_script}", "tf_depends_id", "${var.depends_id}","block-project-ssh-keys", "${var.block_project-ssh-keys}"),
    var.metadata
  )}"
...
resource "google_compute_region_instance_group_manager" "default" {
  ...
  timeouts = {
    create = "${var.http_health_check ? "60m" : "5m"}"
  }
}
